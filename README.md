...
yougram/
    ...
    generated/ <- contains fallback JPEGs, this can be entirely regenerated from your assets so you don't need to keep it safe
...
Yougram officially supports x86_64 Linux, arm64 Linux, and arm64 macOS. It should run on arbitrarily
bad hardware, unless you enable AI features. The yougram binary is around 40MB, because it contains
ffmpeg and maps and such. Qwen AI models range from 500MB to mega huge.
...
can make it work then so can you. It doesn't have zero dependencies, but it also doesn't have very
many and it really is quite easy to set up a dev environment.
